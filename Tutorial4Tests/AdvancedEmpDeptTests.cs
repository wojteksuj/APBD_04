...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(emp => new { EName = emp.EName, Total = emp.Sal + (emp.Comm ?? 0) }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var empDeptJoin = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d });
        var result = empDeptJoin
            .SelectMany(ed => grades.Where(s => ed.e.Sal >= s.Losal && ed.e.Sal <= s.Hisal).Select(s => new { ed.e.EName, ed.d.DName, Grade = s.Grade }))
            .ToList();
